...
@section('content')
    ...
    <a href="{{ route('home') }}" class="font-raguler">Home </a><a href="{{ route('randevu.index') }}"> |
        User randevu list</a>
    <div class="cart-page">
                                ...
                                @foreach ($data as $rs)
                                    ...
                                    <tr>
                                        <td>
                                            <div class="img">
                                                <a href="#"><img
                                                        src="{{ \Illuminate\Support\Facades\Storage::url($rs->treatment->image) }}"
                                                        alt="Image"></a>
                                        ...
                                        <td>{{ $rs->treatment->price }}</td>
                                        <form action="{{route('randevu.update',['id'=>$rs->id])}}" method="post">
                                            @csrf
                                        <td>

                                                <input type="date" name="date" value="{{$rs->date}}" min="{{$rs->date}}"
                                        ...
                                        <td>
                                                ...
                                                <input type="time" name="time" value="{{$rs->time}}" min="{{$rs->time}}"
                                                       onchange="this.form.submit()">
                                                    ...
                                                    @foreach($us->roles as $role)
                                                        @if($role->name=='doctor')
                                                            <option class="nav-link dropdown-toggle"
                                                                    ...
                                                                    aria-expanded="false"
                                                                    value="{{$us->id}}" onchange="this.form.submit()">{{$us->name}}</option>
                                                        @endif
                                                    @endforeach
                                                @endforeach
                                            </select>
                                        ...
                                        <td>
                                            <a
                                                class="icons8-delete" href="{{route('randevu.destroy',[$rs->id])}}"
                            ...
                            <div class="col-md-12">
                                <div class="cart-summary">
                                    <div class="cart-content">
                                        ...
                                        <form action="{{route('randevu.process')}}" method="post">
                                            @csrf
